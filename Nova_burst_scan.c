// clang -Xclang -fopenmp -I/opt/homebrew/include -L/opt/homebrew/lib -lm -lgslcblas -lgsl -L/opt/homebrew/opt/libomp/lib -I/opt/homebrew/opt/libomp/include -lomp   Nova_burst_scan.c -o Nova_burst
...
void MCMCrun(double f_shift, double t_b, double SNR, FILE* bayes_nova);
...
int main(void)
{
    double f_shift, t_b, freq;
    double SNR, SNRmin, SNRmax, SNR_stepsize, SNR_runs, Tobs; 
    freq = 0.0097;
    Tobs = 4.0*year;

    FILE *bayes_nova;

    bayes_nova = fopen("bayes_nova.dat","w");     //clears the file for bayes factors
    fclose(bayes_nova);
    ...
    SNR_stepsize = (SNRmax - SNRmin) / (SNR_runs-1.0);
    
    SNR = SNRmin;
    while(SNR <= SNRmax*1.0005){
        MCMCrun(f_shift, t_b, SNR, bayes_nova);
        SNR += SNR_stepsize;
    }
...
void MCMCrun(double f_shift, double t_b, double SNR, FILE* bayes_nova)
{
    ...
    //This program will not create the folder specified below, if not already in the project folder you will get a segmentation fault
    snprintf(chainpath, 1024, "scanSNR1files/chain_nova_%e_%e_%e_.dat", f_shift*Tobs, t_b, SNR);
    chain_nova = fopen(chainpath,"w");
    ...
    free_double_matrix(paramsx, nc);
    free_double_vector(simp5);
    free_double_vector(simp3);
    free_double_matrix(fish6, np);
    free_double_matrix(fish4, npl);
    free_double_matrix(cov6, np);
    free_double_matrix(cov4, npl);
    free_double_vector(scale6);
    free_double_vector(scale4);
    free_double_vector(ej4);
    free_double_vector(ej6);
    free_double_matrix(ev6, np);
    free_double_matrix(ev4, npl);
    free_double_vector(heat);
    free_int_vector(who);
    free_int_matrix(cnt, nc);
    free_int_matrix(acc, nc);
    free_double_vector(pmap);
    free_double_vector(pml);
    free_double_matrix(history6, nc);
    free_double_matrix(history4, nc);
